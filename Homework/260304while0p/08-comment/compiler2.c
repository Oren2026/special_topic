...
void next_token() {
    while (1) {
        ...
        if (*src == '/') {
            ...
            else if (*(src + 1) == '*') { src += 2; while (!(*src == '*' && *(src + 1) == '/') && *src != '\0') src++; if (*src != '\0') src += 2; continue; }
        }
        break;
    }
...
void statement() {
    if (cur_token.type == TK_IF) {
        ...
        next_token(); next_token(); int jmp_idx = quad_count;
        emit("JMP_F", cond, "-", "?");
        while (cur_token.type != TK_RBRACE) statement();
        next_token(); sprintf(quads[jmp_idx].result, "%d", quad_count);
        ...
        int jmp_false_idx = quad_count;
        emit("JMP_F", cond, "-", "?");
        while (cur_token.type != TK_RBRACE) statement();
        next_token();
        char target[32]; sprintf(target, "%d", loop_start);
        emit("JMP", "-", "-", target);
        sprintf(quads[jmp_false_idx].result, "%d", quad_count);
    } else if (cur_token.type == TK_ID) {
        char name[32]; strcpy(name, cur_token.text); next_token();
