...
| `windows/control/state_machine.py` | `StateMachine` | 模式分派（IDLE/INSTALL/TEST/COMPETE） | `.set_mode(str)` / `.handle_click(u,v)` / `.handle_drag(type,u,v)` |
...
4. TEST 模式（擊球預測）
   使用者依序點擊：POCKET → TARGET_BALL → CUE_BALL
     → ShotDispatcher.add()
     → StateMachine.handle_click() → .get_packet()
     → SocketClient.send()
     ...
     → 回應 PREDICTION
     → HMI._on_wsl_message() → 繪製 ghost ball + 擊球線
