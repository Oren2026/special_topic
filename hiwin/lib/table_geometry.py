...
def distance_mm(x1: float, y1: float, x2: float, y2: float) -> float:
    """兩 mm 點之間的歐氏距離"""
    return ((x2 - x1)**2 + (y2 - y1)**2) ** 0.5


def ball_to_pocket_mm(ball_x: float, ball_y: float,
                       pocket_x: float, pocket_y: float) -> float:
    """球到口袋的 mm 距離（幾何規劃用）"""
    return distance_mm(ball_x, ball_y, pocket_x, pocket_y)
